...
`simulate(1.0, 5.0, 0.0)` contra `coefficientsAt(5.0, 0.0, zeros)` na mesma aeronave, α=5°:
`deflect(None)` devolve `self.normals` bit a bit (`cos(0)=1`, `sin(0)=0` exatos, conferido),
então o caso sem deflexão é exatamente o caso geral com δ=0 — não um caminho de código à
parte. As duas funções montam `V_bar`/`T_a` com a mesma expressão, na mesma aritmética
float32 — não sobra nenhuma conversão de precisão entre elas para explicar diferença
nenhuma, e não sobra: as seis saídas batem bit a bit.

| coeficiente | `simulate` | `coefficientsAt` | \|diferença\| |
|---|---|---|---|
...
Diferença máxima: **0,0e+00** — idêntico bit a bit, não só dentro do ruído de float32.
...
Em vez de confiar num único `h`, varri a largura do passo: diferença finita central em
float32 troca erro de truncamento (h grande) por cancelamento numérico (h pequeno), e o erro
relativo deveria desenhar um V contra `h`, não cair a zero. Um erro decrescente sem piso seria
o sinal de alerta real — indicaria a AD errada concordando por acaso com uma diferença finita
enviesada num único ponto.
...
simétrico deixa os laterais em zero, não `CL`/`Cm`). `J_ad` confirma isso — saem em
~10⁻¹¹, ruído de float32, não sinal real. A métrica de erro relativo restringe-se a `CL` e
`Cm`, as únicas com derivada de verdade; dividir ruído por ruído nos outros componentes só
mediria o piso de precisão do float32, não a AD.

| h (graus) | erro relativo máximo (CL, Cm) |
|---|---|
| 1,0000 | 1,02e-04 |
| 0,1000 | 1,22e-06 |
| 0,0100 | 8,12e-08 |
...
Piso em torno de **8,1e-08** para h≈0,01°, subindo nos dois extremos — o V esperado,
agora medindo o que interessa. Confirma a AD, não invalida.
...
| 5. AD × dif. finita | piso em 8,1e-08 (CL,Cm), V esperado contra h |
